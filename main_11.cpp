...
//노드와 mesh를 분석해서 local Matrix(model)를 계산하고, draw mesh로 넘김(render작업)
void draw_node(const tinygltf::Node& node, kmuvcl::math::mat4f mat_model)
{
  ...
  const std::vector<tinygltf::Mesh>& meshes = model.meshes;
    ...
    //children 분석해서 iter돌면서 children의 node 분석
...
void render_object()
{
    ...
    const std::vector<tinygltf::Accessor>& accessors = model.accessors;
    const std::vector<tinygltf::BufferView>& bufferViews = model.bufferViews;
    // std::cout << "노드개수 " << nodes.size() << std::endl;
...
int main(void)
{
  ...
  window = glfwCreateWindow(700, 700, "Hello Texture with glTF 2.0", NULL, NULL);
  if (!window)
  {
    glfwTerminate();
    return -1;
